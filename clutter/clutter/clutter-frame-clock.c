...
typedef struct _Frame
{
  ...
  gboolean allocated; /* because grefcount doesn't support the value zero */
} Frame;
...
static Frame *
clutter_frame_clock_allocate_frame (ClutterFrameClock *frame_clock)
{
...
static void
clutter_frame_clock_clear_frame (ClutterFrameClock  *frame_clock,
                                 Frame             **frame)
{
  if (frame && *frame)
    {
                                      ...
                                      ClutterFrameInfo  *frame_info)
{
  ...
  clutter_frame_clock_clear_frame (frame_clock,
                                   &frame_clock->prev_presentation);
                              ...
                              int64_t            time_us)
{
  ...
  clutter_frame_clock_clear_frame (frame_clock, &frame_clock->prev_dispatch);
  this_dispatch = frame_clock->prev_dispatch =
    clutter_frame_clock_allocate_frame (frame_clock);
