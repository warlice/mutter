...
G_DEFINE_TYPE (MetaBacklightSysfs, meta_backlight_sysfs, META_TYPE_BACKLIGHT)
                                            ...
                                            GTask              *the_task)
{
  ...
  proc = g_subprocess_new (G_SUBPROCESS_FLAGS_STDOUT_SILENCE,
                           ...
                           g_udev_device_get_sysfs_path (backlight_sysfs->device),
                           brightness_str,
                           NULL);
  if (!proc)
    {
...
on_backlight_changed (MetaUdev           *udev,
                      ...
                      MetaBacklightSysfs *backlight)
{
  if (g_strcmp0 (g_udev_device_get_sysfs_path (udev_device),
                 g_udev_device_get_sysfs_path (backlight->device)) != 0)
    return;
                          ...
                          GError         **error)
{
  ...
  is_internal = meta_output_info_is_laptop_panel (output_info);

  backlight_device = meta_udev_get_backlight_for (udev,
                                                  output_info->name,
                                                  is_internal);
  if (!backlight_device)
    {
  ...
  brightness_path =
    g_build_filename (g_udev_device_get_sysfs_path (backlight_device),
                      "brightness", NULL);
