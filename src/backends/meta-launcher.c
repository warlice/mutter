                                ...
                                GError       **error)
{
  ...
  const char *const active_states[] =
    { "active", "online", NULL };
  g_autofree char *display_session = NULL;
  int saved_errno;
  g_autofree char *type = NULL;
  g_autofree char *state = NULL;

  display_session = get_display_session (error);
  if (!display_session)
    return NULL;

  /* sd_uid_get_display will return any session if there is no graphical
   * one, so let's check it really is graphical. */
  saved_errno = sd_session_get_type (display_session, &type);
  if (saved_errno < 0)
    {
      g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
  ...
  if (!g_strv_contains (graphical_session_types, type))
    ...
    saved_errno = sd_session_get_state (display_session, &state);
    if (saved_errno < 0)
      {
        g_set_error (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND,
  ...
  return get_session_proxy_from_id (display_session, cancellable, error);
}
