  ...
  for (i = 0; i < n_combos; i++)
    {
      if (combo[i].keysym != 0)
        {
          add_keycodes_for_keysym (keys, combo[i].keysym, array);
        }
      else if (combo[i].keycode != 0)
        {
          g_array_append_val (array, combo[i].keycode);
        }
    }
...
reload_iso_next_group_combos (MetaKeyBindingManager *keys)
{
  ...
  get_keycodes_for_combos (keys,
                           ...
                           1,
                           &keys->iso_next_group_combo[0].keycodes,
                           &keys->iso_next_group_combo[0].len);
