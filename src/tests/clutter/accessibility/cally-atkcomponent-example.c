...
main (int argc, char *argv[])
{
  ...
  button3 = clutter_actor_new (context);
  ...
  /* a nested hierarchy, to check that the relative positions are
     computed properly */
  button4 = clutter_actor_new (context);
  ...
  for (i = 0; i < 4; i++) {
    group[i] = clutter_actor_new (context);
